 ...
 * @return void|string
 */
function renderizar($vista, $datos, $layoutFile = "layout.view.tpl", $render = true)
{
    if (!is_array($datos) ) {
 ...
 * @return string
 */
function renderTemplate($template_block, $context, $parent = null, $root = null)
{
    $renderedHTML = "";
    $foreachIsOpen = false;
    $ifIsOpen = false;
        ...
        if (strpos($node, "{{endifnot $currentContext}}") !== false ) {
            if ($ifNotIsOpen) {
                ...
                $renderedHTML .= ($ifNotCondition) ?
                    renderTemplate($innerBlock, $context, $parent, $root): ""
                ;
                $currentContext = "";
                $innerBlock = array();
                $ifNotCondition = false;
